...
function getLineTooltip(args) {
  const { axisSite, yAxisType, digit, labelMap, tooltipFormatter, baseUnit, tooltipHideZero } = args
  ...
  return {
    ...
    formatter(items) {
      ...
      items.forEach(({ seriesName, data, marker }) => {
        ...
        if (tooltipHideZero && itemData === 0) return

        showData = getFormated(itemData, type, digit)
        tpl.push(marker)

        // tooltipHideZero
        tpl.push(`<strong>${seriesName} - ${showData}</strong>` + (baseUnit ? ` ${baseUnit}` : ''))
        tpl.push('<br>')
      })
      return tpl.join('')
    }
...
export const line = (columns, rows, settings = {}, extra = {}) => {
  ...
  const options = { legend, xAxis, series, yAxis, tooltip }
  return options
}
