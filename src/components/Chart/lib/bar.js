...
function getBarTooltip(args) {
  const { axisSite, isHistogram, meaAxisType, digit, labelMap, baseUnit } = args
  ...
  return {
    ...
    formatter(items) {
      ...
      items.forEach(item => {
        ...
        const showData = getFormated(item.value, type, digit)
...
export const bar = (columns, rows, settings = {}, extra = {}) => {
  ...
  const tooltipParams = { axisSite, isHistogram, meaAxisType, digit, labelMap, baseUnit }
