...
export default createDialog({
  ...
  methods: {
    ...
    fetchCustomMove(val) {
      ...
      return this.Fetch.post('V4/web.domain.group.domain.save', {
        group_id: this.id,
        action: 'add',
        domains
      })
    ...
    async fetchLoadList(params, type) {
      const url = type === 'in' ? 'V4/web.domain.group.domain.list' : 'V4/web.domain.group.undistributed.domain.list'
    ...
    async fetchSubmit() {
      ...
      try {
        if (this.mode === 'Create') {
          await this.Fetch.post('V4/web.domain.group.add', form)
        } else {
          form.group_id = this.id
          await this.Fetch.post(`V4/web.domain.group.save`, form)
        }
      } catch (e) {
        throw new Error()
      }
