          ...
          >
            <template slot-scope="scope">
              <ColumnAction>
                ...
                <PopoverConfirm
                  content="确认操作"
                  @submit="handleRowClear(scope.row)"
                >
...
export default {
  ...
  mixins: [DialogRow, consoleData],

  data() {
    return {
      API_INDEX: 'V4/web.domain.group.list',
      selectType: [
        {
          label: '按组名搜索',
          value: 'group_name'
  ...
  methods: {
    ...
    async handleRowClear(row) {
      const { id: group_id } = row
      try {
        await this.Fetch.put('/V4/Web.Domain.DashBoard.saveCache', { group_id })
      } catch (e) {
        return
      }
    ...
    async handleRowDelete(row) {
      const { id: group_id } = row
      try {
        await this.Fetch.post('/V4/web.domain.group.del', { group_id })
      } catch (e) {
        return
      }
      this.fetchList()
