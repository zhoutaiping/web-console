<template>
  <yd-dialog
    :title="title"
    :visible.sync="invoiceConfigVisible"
    ...
    <template v-if="identified">
      ...
      >
        ...
        <template v-if="invoiceForm.invoice_type">
          ...
          >
            <el-input
              v-model="invoiceForm.register_address"
              auto-complete="off"
            />
          </el-form-item>
          <el-form-item
            class="is-required"
            label="注册固定电话"
            prop="register_mobile"
          >
            <el-input
              v-model="invoiceForm.register_mobile"
              auto-complete="off"
            />
          </el-form-item>
          <el-form-item
      ...
      >
        ...
        <el-button
          type="primary"
          @click="handleSubmit('invoiceForm')"
        >确定</el-button>
      </div>
    </template>
    <template v-else>
      <div>
        <div v-loading="loading">
          <span>申请发票前，请先前往<a
            href="/console/user-center#/authentication"
            class="a-link"
          >用户中心</a>完成身份认证</span>
        </div>
        <div
    ...
    initStatus() {
      specialFinanceResource.getApplyInfo().then(response => {
        ...
        if (code === 91064) {
          ...
          this.title = '申请发票'
          const { expressInfo } = response.data.data
          ...
          this.isGovernment = this.isPerson = false
          this.isCompany = true
          // details
    ...
    handleSubmit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$message.warning('请输入的信息有误，请核对后再提交。')
        }
      })
    }
  }
