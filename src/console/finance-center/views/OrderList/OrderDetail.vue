  ...
  <console-page-layout>
    ...
    <div class="module-wrap">
      ...
      <el-row class="module config">
        ...
        <el-col v-if="orderDetails.product_flag !== 'DX' && orderDetails.product_flag !== 'LS'" :span="6">
          <dl>
            <dt>
              购买时长
            </dt>
      ...
      <!-- 支付 -->
      <el-row v-if="!isPaid" class="module">
        <el-col :span="4">
        ...
        <el-col :span="20">
          <ul class="coupon-box">
            <li>
              <el-input v-model.trim="coupon_code" :disabled="useCoupon" placeholder="请输入优惠码"/>
...
export default {
  ...
  created() {
    ...
    this.getOrderDetailById({
      ...
      this.useCoupon = !!this.coupon_code
      // get type list
  ...
  methods: {
    ...
    handlePay() {
      if (this.useCoupon && !this.coupon_code) {
        this.$message.warning('请填写优惠券，并勾选确认')
        ...
        this.$router.push({
          name: 'finance-center.router.orderList__pay',
          query: {
    ...
    handleCancel() {
      this.$confirm('此操作将撤销订单，是否继续？', '提示', {
        ...
        this.cancelOrderByCode({
          ...
          this.$router.push({
            name: 'finance-center.router.orderList'
          })
        })
      }).catch(() => {
        this.$message.warning('已取消撤销订单操作。')
      })
    },
    showPaidStatus() {
